...
import { View, Text, ListRenderItem, TouchableOpacity, ScrollView } from 'react-native';
...
const Slide4: React.FC<SlideComponentProps> = ({ handleNext, updateStepData, onboardingState }) => {
	const {
		filteredData,
		initialData,
		selectedData,
	...
	useEffect(() => {
		if (onboardingState && onboardingState.avoidedIngredients) {
			try {
				...
				const savedFoods = Array.isArray(onboardingState.avoidedIngredients) && 
	...
	return (
		<View style={styles.container}>
			<ScrollView showsVerticalScrollIndicator={false}>
				<Content headerText="next to last question" description="any foods you'd like to avoid?" />
				{selectedData?.length > 0 &&
					<SelectedTags data={selectedData} onClear={(id: string | number) => handleRemoveItem(id)} />}
				<Search data={initialData} handleResetData={handleResetData} onSearchResults={handleFilterData}
						searchKeys={['name']} placeholder="start typing or select from the list below " />
				<ScrollBar style={styles.searchContainer} data={filteredData} renderItem={renderItem}
