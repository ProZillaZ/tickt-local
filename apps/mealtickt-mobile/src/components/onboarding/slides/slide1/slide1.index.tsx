...
const Slide1: React.FC<SlideComponentProps> = ({ handleNext, updateStepData, onboardingState }) => {
	const { state, setState, onChange, errors, isFormValid, shouldShowError, handleSubmitAttempt } = useSlide1();
				...
				<View style={styles.lowerContainer}>
					...
					<View>
						{step1Options.map((option, key) => {
							...
							const inputIndex = inputFields.findIndex(field => field.key === option.key);
							...
							const keyboardType: KeyboardType = isNumericField ? 'numeric' : 'default';
