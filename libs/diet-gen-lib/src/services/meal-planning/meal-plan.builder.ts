...
export class MealPlanBuilder {
  ...
  constructor(userProfile: UserProfile) {
    ...
    this.ingredientSelectionService = new IngredientSelectionService(
      ingredients
    );
    ...
    this.recipeQuantityAdjustmentService =
      new RecipeQuantityAdjustmentService();
  ...
  private calculateTotalWeekMacroAllocation(): MacroAllocation {
    ...
    const bmr = this.caloricIntakeService.calculateBmr(
      gender,
      weightKg,
      heightCm,
      age
    );
    const tdee = this.caloricIntakeService.adjustBmrForActivityLevel(
      bmr,
      activityLevel
    );
    const dailyCalories = this.caloricIntakeService.adjustCaloriesForDietGoal(
      tdee,
      dietGoal
    );
    const dailyMacroAllocation =
      this.macronutrientService.calculateMacroCalories(dailyCalories);
