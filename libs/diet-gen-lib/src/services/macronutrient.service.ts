...
export class MacronutrientService {
     ...
     */
    calculateMacroCalories(totalDayCalories: number, ratio: MacroRatio = DEFAULT_MACRO_RATIO): MacroAllocation {
        if (totalDayCalories <= 0) {
            throw new Error("Total calories must be a positive value.");
        }

        return new MacroAllocation(
            totalDayCalories * ratio.PROTEIN,
            totalDayCalories * ratio.CARBS,
            totalDayCalories * ratio.FATS
        );
    }
    ...
    distribute(
        ...
        mealCount: number = DEFAULT_MEAL_COUNT,
