import { Recipe, Ingredient, RecipeNutritionalInfo } from "@tickt-ltd/types";
...
export class RecipeQuantityAdjustmentService {
  ...
  private calculateScalingFactor(
    currentNutrition: RecipeNutritionalInfo,
    targetMacroAllocation: MacroAllocation
  ): number {
    const targetCalories =
      targetMacroAllocation.proteinCalories +
   ...
   */
  private scaleNutritionalInfo(
    originalNutrition: RecipeNutritionalInfo,
    scalingFactor: number
  ...
  private createAdjustedRecipe(
    ...
    scaledNutrition: RecipeNutritionalInfo,
    scalingFactor: number
  ): Recipe {
  ...
  scaleRecipeForMacros(
    ...
    const scaledNutrition = this.scaleNutritionalInfo(
      recipe.nutritionalInfo,
      scalingFactor
    );

    return this.createAdjustedRecipe(
      recipe,
      scaledIngredients,
      scaledNutrition,
      scalingFactor
  ...
  previewScaling(
    ...
    scalingFactor: number;
    newServings: number;
    projectedNutrition: RecipeNutritionalInfo;
    canScale: boolean;
  } {
    ...
    if (!canScale) {
      return {
        scalingFactor: 0,
        newServings: 0,
        projectedNutrition: new RecipeNutritionalInfo(0, 0, 0, 0, 0),
        canScale: false,
      };
    }
    ...
    const projectedNutrition = this.projectNutritionalInfo(
      recipe,
      scalingFactor
    );
