import {
	...
	NutritionalInfoDto,
	Difficulty,
} from '@tickt-engineering/recipe-types';
import { MealType, DietType, Cuisine } from '@tickt-engineering/nutrition-types';
...
export class RecipeService {
	...
	private convertFirestoreDataToRecipe(data: RecipeFirestoreDocument): Recipe {
		...
		const ingredients = data.ingredients.map((dto, index) => 
			new Ingredient(dto.name, dto.amount, dto.unit, `${data.id}-ingredient-${index}`)
		);
		const instructions = data.instructions.map((dto, index) => 
			new Instruction(dto.stepNumber, dto.description, `${data.id}-instruction-${index}`)
		);
		const tags = data.tags.map((dto, index) => 
			new Tag(dto.name, `${data.id}-tag-${index}`)
		);
	...
	private convertRecipeToFirestoreData(recipe: Recipe, userId?: string): RecipeFirestoreData {
		...
		const nutritionalInfo: NutritionalInfoDto = {
	...
	async getById(id: string): Promise<Recipe | null> {
		const data = await FirestoreUtils.getById<RecipeFirestoreDocument>(this.collection, id);
		if (!data) return null;
	...
	async create(data: CreateRecipeDto, userId: string): Promise<Recipe> {
		try {
			...
			const ingredients = data.ingredients.map((dto, index) => 
				new Ingredient(dto.name, dto.amount, dto.unit, `temp-ingredient-${index}`)
			);
			const instructions = data.instructions.map((dto, index) => 
				new Instruction(dto.stepNumber, dto.description, `temp-instruction-${index}`)
			);
			const tags = data.tags.map((dto, index) => 
				new Tag(dto.name, `temp-tag-${index}`)
			);
			const nutritionalInfo = new NutritionalInfo(
				data.nutritionalInfo.calories,
				data.nutritionalInfo.protein,
			...
			// Store in Firestore
			const stored = await FirestoreUtils.create<RecipeFirestoreDocument>(this.collection, recipeData);
	...
	async update(id: string, data: UpdateRecipeDto, userId: string): Promise<Recipe> {
		...
		const existingData = await FirestoreUtils.getById<RecipeFirestoreDocument>(this.collection, id);
		...
		try {
			...
			const ingredients = data.ingredients ? 
				data.ingredients.map((dto, index) => 
					new Ingredient(dto.name, dto.amount, dto.unit, `${id}-ingredient-${index}`)
				) : existingRecipe.ingredients;
			
			const instructions = data.instructions ?
				data.instructions.map((dto, index) => 
					new Instruction(dto.stepNumber, dto.description, `${id}-instruction-${index}`)
				) : existingRecipe.instructions;
			
			const tags = data.tags ?
				data.tags.map((dto, index) => 
					new Tag(dto.name, `${id}-tag-${index}`)
				) : existingRecipe.tags;
	...
	async search(
		...
		const result = await FirestoreUtils.paginate<RecipeFirestoreDocument>(this.collection, {
			page,
			limit,
			orderBy,
			orderDirection,
		...
		const items = result.items.map(item => this.convertFirestoreDataToRecipe(item));
