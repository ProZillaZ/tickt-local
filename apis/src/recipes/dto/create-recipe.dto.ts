...
import { IsArray, IsEnum, IsNumber, IsOptional, IsString, MaxLength, Min, Max, MinLength, ValidateNested, ArrayMinSize, ArrayMaxSize } from 'class-validator';
...
export class CreateRecipeDto {
	@ApiProperty({ description: 'Recipe name', example: 'Mediterranean Pasta Salad' })
	@IsString()
	@MinLength(1)
	@MaxLength(200)
	name: string;

	@ApiProperty({ description: 'Recipe description', example: 'A refreshing pasta salad with vegetables and feta cheese' })
	@IsString()
	@MinLength(1)
	@MaxLength(2000)
	description: string;
	...
	@ApiPropertyOptional({ description: 'Cuisine types', enum: Cuisine, isArray: true, example: [Cuisine.MEDITERRANEAN] })
	@IsOptional()
	@IsArray()
	@IsEnum(Cuisine, { each: true })
	cuisines?: Cuisine[];

	@ApiProperty({ description: 'Meal types', enum: MealType, isArray: true, example: [MealType.LUNCH] })
	@IsArray()
	@IsEnum(MealType, { each: true })
	@ArrayMinSize(1)
	mealTypes: MealType[];

	@ApiPropertyOptional({ description: 'Diet types', enum: DietType, isArray: true, example: [DietType.VEGETARIAN] })
	@IsOptional()
	@IsArray()
	...
	@ApiProperty({ description: 'Recipe difficulty level', enum: Difficulty, example: Difficulty.MEDIUM })
	@IsEnum(Difficulty)
	...
	@ApiPropertyOptional({ description: 'Recipe image URL', example: 'https://example.com/recipe-image.jpg' })
	@IsOptional()
	@IsString()
	@MaxLength(2000)
	imageUrl?: string;
