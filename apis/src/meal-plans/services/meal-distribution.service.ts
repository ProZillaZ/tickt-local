...
import { MealType, Recipe } from "@tickt-ltd/types";
...
export class MealDistributionService {
  ...
  distributeRecipesForWeek(
    ...
    return this.distributeAcrossDays(
      recipesByMealType,
      mealTypes,
      distributionStrategy
    );
  ...
  getMealTypesForCount(mealCount: number): MealType[] {
    if (mealCount < 1 || mealCount > 6) {
      this.logger.warn(
        `Invalid meal count ${mealCount}, defaulting to 3 meals`
      );
      mealCount = 3;
    }
   ...
   */
  private categorizeRecipesByMealType(
    recipes: RecipeDocument[]
  ): Record<MealType, RecipeDocument[]> {
    return {
      [MealType.BREAKFAST]: recipes.filter((r) =>
        r.mealTypes.includes(MealType.BREAKFAST)
      ),
      [MealType.LUNCH]: recipes.filter((r) =>
        r.mealTypes.includes(MealType.LUNCH)
      ),
      [MealType.DINNER]: recipes.filter((r) =>
        r.mealTypes.includes(MealType.DINNER)
      ),
      [MealType.SNACK]: recipes.filter((r) =>
        r.mealTypes.includes(MealType.SNACK)
      ),
    };
  }
    ...
    for (let day = 0; day < 7; day++) {
      ...
      for (let meal = 0; meal < mealTypes.length; meal++) {
        ...
        if (availableRecipes && availableRecipes.length > 0) {
          ...
          this.logger.warn(
            `No ${mealType} recipes available for day ${day + 1}`
          );
        }
      }
  ...
  private selectRecipeForMeal(
    ...
    switch (strategy) {
      case "balanced":
        return this.selectBalancedRecipe(
          availableRecipes,
          day,
          meal,
          totalMeals
