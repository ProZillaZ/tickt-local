 ...
 */
export class FirestoreMealPlanService
  extends BaseService
  implements IMealPlanService
{
   ...
   */
  private convertMealPlanToData(
    mealPlan: CreateMealPlanDto | UpdateMealPlanDto
  ): any {
    const now = new Date();
  ...
  async getById(id: string, userId: string): Promise<WeekMealPlan | null> {
    try {
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.getById"
      );
    }
  }
  ...
  async create(data: CreateMealPlanDto, userId: string): Promise<WeekMealPlan> {
    try {
      ...
      const newId = await this.dbAdapter.create(
        userCollectionPath,
        mealPlanData
      );
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.create"
      );
    }
  }
    ...
    try {
      ...
      if (data.startDate || data.endDate) {
        const startDate = new Date(
          data.startDate || existingMealPlan.startDate
        );
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.update"
      );
    }
  }
  ...
  async delete(id: string, userId: string): Promise<void> {
    try {
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.delete"
      );
    }
  }
  ...
  async getByUser(
    ...
    try {
      ...
      const queryResult = await this.dbAdapter.query(
        userCollectionPath,
        queryOptions
      );
      ...
      const result = this.createPaginationResult(
        items,
        limitCount,
        queryResult.lastDoc
      );
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.getByUser"
      );
    }
  }
  ...
  async getByDateRange(
    ...
    try {
      ...
      this.logSuccess("getByDateRange", {
        userId,
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.getByDateRange"
      );
    }
  }
  ...
  async getCurrentWeekMealPlan(userId: string): Promise<WeekMealPlan | null> {
    try {
      ...
      const mealPlans = await this.getByDateRange(
        userId,
        startDateStr,
        endDateStr
      );
      ...
      const currentWeekPlan = mealPlans.find(
        (plan) => plan.startDate <= startOfWeek && plan.endDate >= endOfWeek
      );

      this.logSuccess("getCurrentWeekMealPlan", {
        userId,
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.getCurrentWeekMealPlan"
      );
    }
  }

  async searchMealPlans(
    userId: string,
    ...
    try {
      ...
      throw this.errorHandler.handleError(
        error as Error,
        "FirestoreMealPlanGroupedService.searchMealPlans"
      );
    }
  }
}
